 ...
 * 理解递归细节:
 *
 * 将递归调用前的代码想象成 "沿着树向下走": 它将给定的键和每个结点的键比较, 根据
 * 比较结果决定向左还是向右移动到下一个结点. 将递归调用后的代码想象成 "沿着树向
 * 上爬": 对于 get(), 对应于一系列的 return 语句;对于 put(), 意味着重置搜索路径
 * 上每个父结点指向子结点的链接, 并增加每个结点中的计数器的值.
 *
 * 二叉查找树的运行时间取决于树的形状 (是否平衡), 而树的形状又取决于键被插入的先
 * 后顺序.
 *
 * 二叉查找树和_快速排序_ (@see QuickSort) 几乎就是 "双胞胎": 树的根节点就是快速
 * 排序中的第一个切分元素, 而这对于所有的子树同样适用, 这和快速排序中对子数组的
 * 递归排序完全对应. 这使我们能够分析得到二叉查找树的一些性质.
 *
 * 命题: @see p.255
 *
...
class BST
{
     ...
     *
     * 和二分查找中每次迭代之后查找区间就会减半一样, 不断向下查找, 当前结点所表
     * 示的子树的大小也在减小一次查找也就定义了树的一条 **路径**.
     */
    ...
    /**
     * 如果 key 存在于以 x 为根结点的子树中则更新它的值, 否则将以 key 和 val 为
     * 键值对的新结点插入到该子树中.
     *
     * 这段代码几乎和二分查找一样简单, 这种简洁性就是二叉查找树的重要特性之一另
     * 一个更重要的特性就是插入的实现难度和查找差不多.
     */
     ...
     * 
     *   这个方法在删除结点 x 后用它的 "后继结点" 填补它的位置. 因为 x 有一个右
     *   子结点, 因此它的后继结点就是其右子树中的最小结点. 因为这个后继结点大于
     *   x 左子树中所有结点, 且小于 x 右子树中所有结点, 故用其替换 x 之后仍能保
     *   持有序性
     *
     *   它的缺点是可能会在实际应用中产生性能问题. 这个问题在于选用后继结点是一
     *   个随意的决定, 且没有考虑树的对称性.
     */
     ...
     *                         M
     *
     * 注意这个方法, 实际上两次使用了 deleteMin 的处理逻辑, 一个是处理被删除的结
     * 点仅有一个子结点或没有子结点的; 另一个是处理被删除结点有两个子结点, 用于
     * 防止出现空悬结点. 如示例中的 M, 使用 deleteMin 被挂到了 R.left, 防止了 H
     * 在替代了 E 后, M 因为没被处理从而空悬被回收.
     */
