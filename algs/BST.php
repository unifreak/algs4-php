 ...
 *
 * 二叉查找树和_快速排序_几乎就是 "双胞胎": 树的根节点就是快速排序中的第一个切分元素, 而这对于所有
 * 的子树同样适用, 这和快速排序中堆子数组的递归排序完全对应. 这使我们能够分析得到二叉查找树的一些性质
 *
 * 命题: @see p.255
 * - C: 由 N 个随机件构造的二叉查找树中, 查找命中平均所需的比较次数为 ~2lnN (约 1.39lgN)
 * - D: 由 N 个随机件构造的二叉查找树中, 插入操作和查找未命中平均所需比较次数为 ~2lnN (约 1.39lgN)
...
class BST
{
    ...
    {
        // 不断检索左子树直至遇见空的左链接, 返回该结点的右链接. 左链接会被当做垃圾回收
    ...
    {
        // 不断检索右子树直至遇见空的右链接, 返回该结点的左链接. 右链接会被当做垃圾回收
...
// @todo Node Class Node->STNOde->BSTNode?
class BSTNode
{
    public $key; // 键
    public $val; // 值
