...
原书中的代码全部用 Java 实现. 我在阅读过程中把其中的 Java 代码转译成了 PHP 实现. 一方面是为了通过手动敲代码, 加深对内容的理解.
另一方面, 也是为了方便自己以后 PHP 的算法面试复习.
...
你应该把 Wiki 作为大纲, 一遍阅读 Wiki, 一遍参阅相关代码. 你可以克隆一份项目代码到本地,
通过实际运行算法实现, 或者修改算法实现加深理解. 算法的主要思路, 主要在代码注释中.
...
`while` 循环中, 递增变量在循环结束之后仍然是可用的. 这个区别常常是使用 `while` 而非 `for` (不可用) 的主要原因.
...
_增长数量级_ 则让我们更进一步: 将程序和它实现的算法 _隔离_ 开来, 这使我们对算法性能的知识可以应用于任何计算机.
...
常见增长数量级的分类
    ...
    ---------------------------------------------------------------------------
    常数     1         普通语句    两数相加
    对数     logN      二分策略    二分查找      对数的底数和增长的数量级无关
                                                 因为不同底数仅相当于一个常数因子
    线性     N         循环        找出最大元素
    线性对数 NlogN     分治        归并排序      在实践中非常重要
    平方     N^2       双层循环    检查所有元素对
    立方     N^3       三层循环    检查所有三元组
    指数     2^N       穷举        检查所有子集   非常慢, 不可能用于解决大规模问题
                                                  但在算法理论仍有重要地位
...
倍率定理: 如果 T(N)~aN^blgN, 那么 T(2N)/T(N)~2^b.
...
初级实现: _插入_ 和 _删除最大元素_ 在最坏情况下需要 _线性_ 时间.
...
- _命中_ 表示一次成功的查找, _未命中_ 表示一次失败的查找
...
![2-3 tree](./img/2-3tree.png "2-3 tree")

我们将指向一棵空树的链接称为**空连接**.
一颗**完美平衡**的 2-3 查找树中的所有空链接到根节点的距离应该是相同的.
我们用 _2-3 树_ 指代一颗完美平衡的 2-3 查找树

2-3 树的分析和二叉查找树的分析大不相同, 我们主要感兴趣的是_最坏情况下_的性能, 而非_一般情况_ (这种
情况下我们会用随机键模型分析预期的性能)

查找: 先将它和根结点中的键比较。如果它和其中任意一个相等, 查找命中;否则我们就根据
比较的结果找到指向相应区间的链接, 并在其指向的子树中递归地继续查找.
如果这是个空链接, 查找未命中.

插入新键:

- 到2-结点中: 把这个2-结点替换为一个3-结点, 将要插入的键保存在其中
- 到3-结点中:
    + 是树中唯一结点: 先将新键存入该结点中使之成为一个4-结点, 然后将它转换为一棵由3个2-结点组成的2-3树.
      这保持了树的平衡, 也说明了2-3树是如何生长的.
    + 父结点为2-结点: 构造4-结点并将其分解, 将其移动至原来的父结点中.
      这是2-3 树的动态变化的核心.
    + 父结点为3-结点: 构造4-结点并分解, 将它的中键插入它的父结点中. 在结点上进行相同的分解变换, 一直向上
      不断分解直至遇到一个2-结点并将它替换为一个不需要继续分解的3-结点, 或者是到达3-结点的根
...
- 每个变换都会将4-结点中的一个键送入它的父结点中，并重构相应的链接而不必涉及树的其他部分.
- 这些局部变换不会影响树的全局有序性和平衡性

分解根结点: 按照向一棵只有一个3-结点的树中插入新键的方法处理这个问题

分解4-结点的所有情况:

![4-node split](./img/4-node_split.png "4-node split")
...
尽管可以用不同的数据类型表示2-结点和3-结点, 但这种直白的表示方法实现大多数的操作并不方便, 因为
...
用这种方式表示 2-3 树的二叉查找树称为 **红黑二叉树查找树** (简称 **红黑树**)
- 红链接均为左链接
- 没有任何一个结点同时和两条红链接相连
- 该树是完美黑色平衡的, 即任意空链接到根结点的路径上的黑链接数量相同

可以看到, 将红链接画平时, 一颗红黑树就是一颗 2-3 树.
红黑树既是二叉查找树, 也是 2-3 树.
它将二叉查找树中简洁高效的查找方法和 2-3 树中高效的平衡插入算法结合了起来

![RBtree](./img/RBtree.png "Red Black Tree")
...
![2-node insertion](./img/insert_2node_only.png "2-Node Insertion")

- 在树底的 2-结点

![2-node insertion](./img/insert_2node_bottom.png "2-Node Bottom Insertion")

向 3-结点中插入 (动态变化的关键)

![3-node insertion](./img/insert_3node.png "3-Node Insertion")

红链接的向上传递

![Red Link](./img/red_link.png "Red Link")
...
2. _处理碰撞冲突_:
- _拉链法_ @see SeparateChianingHashST
- _线性探测法_ @see LinearProbingHashST
...
假设 p.296
- J (均匀散列假设): 我们使用的散列函数能够均匀独立的将所有的件散布于 0 到 M-1 之间
...
**集合** (SET): 无需处理具体值, 只需检测键在表中是否存在. 分为有序和无序两个版本. 用例如:

- 去掉输入流中重复项
- 用一个文件中保存的键来判定输入流中的哪些键可以被传递到输出流: 白名单, 黑名单等

## 字典类应用

将符号表看做一个可以方便地查询并更新其中信息的动态字典, 每个键都有一个与之关联的值.
...
一个键和多个值相关联的符号表 @todo LookUpIndex, FileIndex.
...
**图**是由一组_顶点_和一组能够将两个顶点相连的_边_组成的.

一般用 0 至 V-1 表示一张含有 V 个顶点的图的各个顶点.
用 v-w 表示连接 v 和 w 的边, w-v 是这条边的另一种表示方法.
**自环** 即一条连接一个顶点和其自身的边. 连接同一对顶点的两条边称为**平行边**.
...
如果从任意一个顶点都存在一条路经到达另一个任意顶点, 称这幅图是**连通图**.
一幅**非连通的图**由若干连通的部分组成, 它们都是其**极大连通子图**.
一般来说, 要处理一张图就需要一个个的处理它的连通分量 (子图).

**树**是一幅无环连通图. 当且仅当一副含有 V 个结点的图 G 满足下列 5 个条件_之一_时, 它是一棵树
- G 有 V-1 条边且不含有环
- G 有 V-1 条边且是连通的
- G 是连通的, 但删除任意一条边都会使它不再连通
- G 是无环图, 但添加任意一条边都会产生一条环
- G 中的任意一对顶点之间仅存在一条简单路径
以上这些性质在分析和实现寻找生成树和森林算法时扮演着重要角色

互不相连的树组成的集合称为**森林**.
连通图的**生成树**是它的一幅子图, 它含有图中所有顶点且是一棵树.
图的**生成树森林**是它的所有连通子图的生成树的集合.

图的**密度**指已经连接的顶点对占所有可能被连接的顶点对的比例.
在**稀疏图**中, 被连接的顶点对很少; 而在**稠密图**中, 只有少部分顶点队之间没有边连接.
我们将会遇到的应用使用的几乎都是稀疏图.
**二分图** 是一种能够将所有结点分为两部分的图, 其中图的每条边所连接的两个顶点都分别属于不同的部分 (比如电影和
演员关系图).
...
- **邻接表数组**: 以顶点为索引的列表数组, 其中的每个元素都是和该顶点相邻的顶点列表.
    ...
    ![adj table](./img/adj_tbl.png "Adjecent Table")

深度优先算法 (DFS) 是本章好几种图算法的基础, 这种简单的递归模式只是一个开始, DFS 能有效处理许多和图有关的任务:

- 连通性: 两个给定的顶点是否连通 (路径检测) @see Search, DepthFirstSearch
- 单点路径 / 最短路径: 找出从 s 到给定目的顶点 v 的路径 @see Paths, DepthFirstPaths
- 连通分量: 找出一幅图的所有连通分量 @see CC
- 检测环: 给定的图是无环图吗 @see Cycle
- 双色问题: 能够用两种颜色将图中所有顶点着色, 使得任意一条边的两个端点的颜色都不相同吗.
  即这是一副二分图吗 @see TwoColor

最短路径问题: 但 DFS 遍历整个图的顺序和找出最短路径的目标没有任何关系.

解决最短路径问题的经典方法是 **广度优先搜索** (BFS) @see BreadthFirstPaths:

    要找到从 s 到 v 的最短路径, 从 s 开始, 在所有由一条边就可以达到的顶点中寻找 v, 如果找不到我们就继续在与 s 距离两条边的所有顶点中查找
    v, 如此一直进行. DFS 好像一个人在走迷宫, BFS 则好像是一组人在一起朝各个方向走出这座迷宫

广度优先算法 (BFS) 能帮助解决以下问题:

- 最短路径 @see BreadthFirstPaths
- 间隔度数 @see SymbolGraph, DegreeOfSeparation
  间隔度数的理论在计算机网络的设计以及理解各个学科领域中的自然网络中都能起到重要的作用

**符号图**: 用字符串而非整数来表示和指代顶点.
...
**有向路径**由一系列定点组成, 对于其中的每个顶点都存在一条有向边从它指向序列中的下一个顶点.

**有向环** 为一条至少含由一条边且起点和终点相同的有向路径.
...
我们用跟无向图相同的邻接表来表示一个有向图. 不同的是有向图中每条边都只会出现一次.
这个区别在有向图的处理中影响深远
...
保留一个位做垃圾收集之用. 它会周期性地运行一个类似于 DirectedDFS 的有向图可达性算法来标记所有可以
被访问到的对象, 然后清理所有对象, 回收没有被标记的对象

**寻路**:
@see DepthFirstPaths -> DepthFirstDirectedPaths
...
实际应用中, 拓扑排序和有向环检测总是一起出现. 因为有向环的检测是拓扑排序的前提.
因此解决任务调度类的问题通常需要三步:
...
对于无向图, 这个问题等价于连通性问题. 对于有向图, 它和强连通性的问题有很大区别, 可以用传递闭包解决.
...
用远小于平方级別的空间支持常数级別的查询的一般解决方案仍然是一个有待解决的研究问题, 并且有重要的实际意义:
例如, 除非这个问题得到解决, 对于像代表互联网这样的巨型有向图, 否则无法有效解决其中的顶点对可达性问题.

## 问题和对应算法一览

	无向图   连通性            DFS     DepthFirstSearch
	        单点路径          DFS     DepthFirstPaths
	        连通分量          DFS     CC
	        环检测            DFS     Cycle
	        二分图/双色问题    DFS     TwoColor
	        最短路径/间隔度数  BFS    BreadthFirstPaths
	有向图   路径             DFS    DepthFirstDirectedPaths
	        最短路径          BFS     BreadthFirstDirectedPaths
	        环检测            DFS      DirectedCycle
	        拓扑排序          DFS     Topological
	        可达性            DFS      TransiveClosure
...
@see Edge, EdgeWeightedGraph, EdgeWeightedDigraph
...
- 边的权重可能是 0 或者负数
- 所有边的权重都各不相同: 如果边的权重可以相同, MST 就不一定唯一了, 这会使算法证明变复杂. 实际上
...
- **Kruskal 算法**: 主要思想是按照边的权重 (从小到大) 处理他们. 将边加入 MST 中, 加入的边不会
  与已经加入的边构成环, 直到树中含有 V-1 条边为止. 这些加入的边逐渐由一片森林合并为一棵树, 也就是
  MST @see KruskalMST
